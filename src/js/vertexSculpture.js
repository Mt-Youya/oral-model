...
function vertexSculpture(publicAPI, model) {
  ...
  publicAPI.requestData = (inData, outData) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      let found = false
      for (const idx of removingPoints) {
        if (poly.includes(idx)) {
          found = true
          break
        }
      }
  ...
  publicAPI.setCellInfo = (cellIdx, toothIdx) => {
    ...
    if (!scalars) {
      ...
      scalars.getData().fill(undefined)
